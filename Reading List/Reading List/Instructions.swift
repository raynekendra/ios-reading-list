...
/*
 ...
 Create a "BookController.swift" file, and create a BookController class inside of it.
 Add a variable called books and set its value to an empty array of Book objects.
 ...
 Drag out a UITableViewController scene, and embed it in a navigation controller. Set the navigation controller as the initial view controller. Then, drag out a UIViewController scene that will serve as the detail view controller.
 Set the table view controller's title to be "Reading List"
 In the table view's cell:
 ...
 Add a bar button item in the top-right corner of the table view controller. Set its "System Item" to "Add". Create a segue from it to the detail view controller. Give the segue an identifier.
 Create a Cocoa Touch subclass for both the table view controller and the custom UITableViewCell. Call the UITableViewController subclass ReadingListTableViewController, and the UITableViewCell subclass BookTableViewCell. Set the table view controller and cell's class in the Identity inspector.
 In the custom cell class, create outlets from the label and button, and an action from the button as well.
 ...
 Add a text field and a text view to the detail view controller scene. They will be used to show, create, and edit a Book's title and reasonToRead strings respectively. Constrain them with the keyboard in mind so that they won't get covered by it when editing the fields.
 Add a navigation item to the detail view controller, then add a bar button item to the top-right corner of the view controller. Set its "System Item" to "Save"
 Create a Cocoa Touch Subclass of UIViewController called BookDetailViewController and set this view controller's class to it in the Identity Inspector.
 Add outlets from the text field and text view, and an action from the save button.
 Part 3 - Wiring Everything Up

 BookTableViewCell

 Add a var book: Book? variable to the BookTableViewCell class.
 If you haven't done so already, create outlets from the label and button, and an action from the button from the storyboard.
 ...
 Fill out the titleForHeaderInSection function, giving each section a unique title.
